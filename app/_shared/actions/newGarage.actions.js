...
export const SET_GARAGE_LAT          = "SET_GARAGE_LAT"
export const SET_GARAGE_LNG          = "SET_GARAGE_LNG"
...
export function setLat (lat){
  return  { type: SET_GARAGE_LAT
          , value: lat
          }
}
export function setLng (lng){
  return  { type: SET_GARAGE_LNG
          , value: lng
          }
}
...
export function submitNewGarage(state) {
  return (dispatch, getState) => {
    ...
    if (state.id == undefined) { // new garage
      request( onSuccess
             , CREATE_NEW_GARAGE
             , { garage: {name: state.name, address: state.address, lat: state.lat, lng: state.lng, floors: newFloors}}
             , "garageMutations"
             )
    } else { // garage edit
      request( onSuccess
             , UPDATE_GARAGE
             , {id: state.id, garage: { name: state.name, address: state.address, lat: state.lat, lng: state.lng, floors: newFloors}}
             , "garageMutations"
             )
    }
...
function changeFloorValue(name, value, index){
  return (dispatch, getState) => {
    ...
    if (newData[newData.length-1].label!='' || newData[newData.length-1].scheme!='') newData = insertEmptyRow(newData)
    dispatch(setFloors(newData))
  }
}
...
export function initEditGarage(id){
  return (dispatch, getState) => {
    const onSuccess = (response) => {
      ...
      dispatch(setLat(response.data.garage.lat))
      dispatch(setLng(response.data.garage.lng))
