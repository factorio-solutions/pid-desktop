...
export function initUsers() {
  return (dispatch, getState) => {
    const onSuccess = (response) => {
      ...
      let pendingClients = response.data.client_users.filter(filterPending).map(cu => ({ ...cu.user
                                                                                       , client: { ...cu.client
                                                                                                 , admin: cu.admin
                                                                                                 , secretary: cu.secretary
                                                                                                 , internal: cu.internal
      ...
      let pendingCars = response.data.user_cars.filter(filterPending).map(uc => ({ ...uc.user
                                                                                 , car: { ...uc.car
                                                                                        , admin: uc.admin
                                                                                        }
      ...
      let pendingGarages = response.data.user_garages.filter(filterPending).map(gu => ({ ...gu.user
                                                                                       , garage: { ...gu.garage
                                                                                                 , admin: gu.admin
