    ...
    return (
      <PageBase>
        <Form onSubmit={submitForm} submitable={checkSubmitable()} onBack={goBack} onHighlight={actions.toggleHighlight}>
          ...
          <LabeledRoundButton label={t([ 'finance', 'uploadKey' ])} content={<span className="fa fa-file-code-o" aria-hidden="true" />} onClick={() => { document.getElementsByName(`newAccountPrivateKey`)[0].click() }} type={state.csob_private_key === '' ? 'action' : 'confirm'} />
        </Form>
      </PageBase>
    )
  }
