...
class GoInternalPage extends Component {
  ...
  componentDidMount() {
    const { actions, pageBase: { garage } } = this.props
    garage && actions.initGoInternal()
  }

  componentWillReceiveProps(nextProps) { // load garage if id changed
    const { actions, pageBase: { garage } } = this.props

    nextProps.pageBase.garage !== garage && actions.initGoInternal()
  }
...
const enhancers = compose(
  withMasterPageConf(toAdminModules('goInternal')),
  connect(
    state => ({ state: state.adminGoInternal, pageBase: state.pageBase }),
    dispatch => ({ actions: bindActionCreators(goInternalActions, dispatch) })
  )
)

export default enhancers(GoInternalPage)
