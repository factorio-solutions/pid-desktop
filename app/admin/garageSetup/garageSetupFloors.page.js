...
class GarageSetupFloorsPage extends Component {
  ...
  componentDidMount() {
    ...
    } else if (state.tarif_id === undefined || state.name === '' || state.city === '' || state.line_1 === '' || state.postal_code === '' || state.country === '') {
      this.goBack()
    }
  }
  ...
  render() {
    ...
    const checkSubmitable = () => {
      if (state.floors.find(floor => floor.label === '' || floor.scheme === '') !== undefined) return false
      ...
      return (
        <div key={index}>
          <div className={styles.inline}>
            ...
            />
            <Input
              style={styles.smallInputWidth + ' ' + styles.middleMargin}
              onChange={handleFloorFromChange}
              label={t([ 'newGarage', 'from' ])}
              error={t([ 'newGarage', 'invalidFloorFrom' ])}
              value={floor.from} name={`floor${index}[from]`}
