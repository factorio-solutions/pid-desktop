...
class UsersPage extends Component {
  ...
  render() {
    ...
    const createCarLink = (car, index) => (<li key={index} className={styles.clickable} onClick={() => { this.carOnClick(car.id) }}> {car.model}
    ...
    const usersData = (state.filter === 'all' ? state.users : state.pending).map(user => ({
      ...user,
      spoiler: state.filter === 'all' ? (<div className={styles.spoiler}>
        <div>
    ...
    return (
      <PageBase>
        ...
        <div className={styles.addButton}>
          <RoundButton content={<span className="fa fa-plus" aria-hidden="true" />} onClick={this.toInviteUser} type="action" size="big" />
        </div>
      </PageBase>
    )
  }
