...
class NewContractPage extends Component {
  ...
  render() {
    ...
    const prepareClients = client => ({ label: client.name, onClick: actions.setClient.bind(this, client.id) })
    const prepareCurrencies = currency => ({ label: currency.code, onClick: actions.setCurrency.bind(this, currency.id) })
    const prepareRents = rent => ({ label: rent.name, onClick: actions.setRent.bind(this, rent) })
