...
export default class Table extends Component {
  ...
  render() {
		...
		const { compareRepresentations, comparator, representer } = _.find(schema, (s)=>s.key==sortKey)
		...
		return (<table className={styles.rtTable}>
			<thead>
				<tr>
					{schema.map((value,key)=>{
						return (<td key={key} onClick={()=>{handleHeadClick(key)}} className={styles.tdHeader}>
								{value.title}
			...
			<tbody>
				{data.map((value,key)=>{
					return [<TableRow key={key} className={`${spoilerId == key && styles.spoilerRow} ${value.disabled && styles.disabled}`} schema={schema} data={value} onClick={()=>{handleRowClick(key)}} hover/>,
