...
const createWeeks = (month, selected, onClick) => {
  ...
  return days.map((week, key) => {
    ...
    return (
      <tr key={key} className={styles.pickerRow}>{week.map(createDays)}</tr>
    )
  })
}
...
export default function Days({ month, selected, onClick }) {
  return (
    <table className={styles.daysTable}>
      ...
      <tbody>
        {createWeeks(month, selected, onClick)}
      </tbody>
    </table>
  )
}
